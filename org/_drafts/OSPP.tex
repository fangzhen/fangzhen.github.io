...
\title{读书笔记}
\subtitle{操作系统：原理与实践}
\AtBeginSubsection[]{\begin{frame}\frametitle{}\setcounter{tocdepth}{2}\tableofcontents[currentsubsection,sectionstyle=show/shaded,subsectionstyle=show/shaded/hide]\end{frame}}
\hypersetup{
 pdfauthor={},
 pdftitle={读书笔记},
 pdfkeywords={},
...
\section{第一卷 内核与进程}
\label{sec:orga77bb8d}
\subsection{引言}
\label{sec:org14c0d42}
\begin{frame}[label={sec:org0381675}]{引言}
...
\begin{frame}[label={sec:org083b705}]{操作系统定义}
操作系统是一个软件层，管理计算机资源，为用户及其应用程序提供服务。
\end{frame}

\begin{frame}[label={sec:org6939fdc}]{操作系统角色}
...
\begin{frame}[label={sec:orgdf33c4c}]{其他系统的类似角色}
...
\begin{frame}[label={sec:org4bf7c15}]{操作系统评估标准：}
\begin{itemize}
...
\begin{frame}[label={sec:orgd9b17cf}]{操作系统：过去现在与未来}
\begin{itemize}
...
\subsection{内核抽象}
\label{sec:orgc85c8e1}

\begin{frame}[label={sec:org6c2884d}]{概述}
\begin{itemize}
...
\begin{frame}[label={sec:org2dc6fad}]{进程抽象}
\begin{itemize}
\item 进程是程序运行实例，
\begin{itemize}
\item 执行指令
...
\begin{frame}[label={sec:orga9108a9}]{进程保护}
...
\begin{frame}[label={sec:org1e7603f}]{模式切换}
\begin{itemize}
\item 用户态到内核态
\begin{itemize}
\item 外部中断
...
\begin{frame}[label={sec:org11e5cd8}]{实现安全的模式切换}
\begin{itemize}
...
\begin{frame}[label={sec:orge3d20f7}]{具体实现}
\begin{itemize}
\item x86中断处理的模式切换
\item 系统调用的实现
\item 创建新进程
...
\begin{frame}[label={sec:orge3259b9}]{Case Study}
\begin{itemize}
